...
import io.openlineage.spark.builtin.scala.v1.InputDatasetWithDelegate;
import io.openlineage.spark.builtin.scala.v1.InputDatasetWithFacets;
import io.openlineage.spark.builtin.scala.v1.InputDatasetWithIdentifier;
import io.openlineage.spark.builtin.scala.v1.InputLineageNode;
...
@Slf4j
public class SparkBuiltInInputDatasetBuilder
    extends AbstractQueryPlanInputDatasetBuilder<LogicalPlan> {

  public SparkBuiltInInputDatasetBuilder(OpenLineageContext context) {
    super(context, false);
  }
  ...
  protected List<InputDataset> apply(SparkListenerEvent event, LogicalPlan x) {
    ...
    List<InputDatasetWithFacets> datasets =
        ScalaConversionUtils.<InputDatasetWithFacets>fromSeq(
            lineageNode.getInputs(BuiltInPlanUtils.context(context)).seq());
