...
public class ExpressionDependencyCollector {
  ...
  static void collectFromNode(
    ...
    if (node instanceof ColumnLevelLineageNode) {
      builtInColumnLineage(context, builder, (ColumnLevelLineageNode) node);
  ...
  private static void builtInColumnLineage(
    ...
    List<ExpressionDependency> deps =
        ScalaConversionUtils.<ExpressionDependency>fromSeq(
            node.columnLevelLineageDependencies(BuiltInPlanUtils.context(context)).seq());
    ...
    deps.stream()
        ...
        .forEach(
            d ->
                ScalaConversionUtils.<OlExprId>fromSeq(d.inputExprIds().seq()).stream()
                    .forEach(
                        i ->
